...
- `--next HH:MM` — plain pin: hold every other activation until T, fire once as `reason: "override"`, clear on success. Earlier or later than the next slot; the times list is untouched. On interval the chain renews from that fire, so a fresh connection's first anchor can be deferred this way too.
- `--next HH:MM --move-slot` — the slot-move special case, and **the replacement for `--start`** (which is gone): on fixed, `next_pending_slot` names the earliest still-reachable incomplete slot and the pin fires *that slot* at T (slot identity preserved, slot marked completed). Interval has no slot to name — plain `--next` already renews the chain from the fire. Works on delegated connections.
- `--clear-next` drops the pin. Setting a new one replaces the old.
- A moved slot now completes — or, when its catch-up ladder exhausts, skips — on **its own calendar day**. A `--move-slot` pin set before midnight for a slot living on a future day used to book the completion on the pin's landing day, so the original slot fired twice: once at the pin, again on its real day. `nextOverrideSlotDay` carries the slot's day through the pin, the CLI, and the server endpoint.
- Remote path: `POST /v1/connections/<id>/override` with `{"nextOverrideAt": iso|null, "nextOverrideSlot": "HH:MM"|null, "nextOverrideSlotDay": "YYYY-MM-DD"|null}` — state only, never a connection re-push, so schedule memory survives. The endpoint now validates its inputs instead of trusting them: the slot must be one of a fixed-mode connection's fixed times, and the day must be `YYYY-MM-DD` (and only naming a slot carries a day). Solo `serve` and `awewarm-hub` both expose it (hub re-authorizes the tenant first); both are backward compatible with older clients that omit the day.
- Failures retry under the existing 5-minute throttle; the health ladder remains the brake. `status` shows `Pinned: …` — `slot HH:MM (moved from DAY)` for a moved slot armed for another day, plus a "held while auto-disabled" note — and the pin is armed into the RTC wake layer.
- Old `deferUntil` values migrate into a plain pin on first read (no slot identity survived the old field). Behavior change vs the old gate: a moved slot now fires at T even past the 30-minute catch-up ceiling (catch-up only applied to ungated late fires).

**`status` hides user-disabled connections by default; `--all` lists them.** The default listing (and `--json`) skipped only `hide`, so a parked connection like a spare account still cluttered every status run. Listing now also drops `enabled: false` — same contract as hide: an explicit id still shows it, `--all` brings disabled ones back for scripts and debugging, and auto-disabled (health-ladder) connections stay visible because they need attention, not a quiet hide. Empty listings now say what is hiding what: hidden-only points at `--show`, disabled-only at `--on`/`--all`, a mix names both — and `status --local` no longer claims "every connection is delegated" while invisible local connections (disabled or hidden) exist; it prints the reason nothing local is visible instead.

Two smaller fixes: `awewarm status --help` carried a literal `\b` that pushed its examples into Click's no-rewrap block, overflowing narrow terminals (the wrap-aware help layer added in v0.6.8 now handles them, and `serve`'s docstring indent is aligned); and a label that already names its account — "Codex (cxo-heck)" — no longer renders with duplicate parens when the id suffix is appended.
...
**`config proxy` becomes one switch for every egress awewarm triggers — including the CLI subprocesses it starts.** The `proxyUrl` used to route only awewarm's own HTTPS, leaving codex/claude subprocess fires on the ambient environment: on a network that genuinely requires a proxy, subscription warm-ups could work while every account fire failed, with the egress hint describing only half the picture. Setting `awewarm config proxy <url>` now also injects `http_proxy`/`https_proxy`/`HTTP_PROXY`/`HTTPS_PROXY` into the codex/claude subprocesses awewarm spawns — the explicit URL wins over ambient proxy variables, and the rest of the child environment (delegated logins, `authHome` pointers, `PATH`, the codex sandbox) rides along unchanged. Unset, behavior is unchanged: no overlay is built, a locally fired CLI with no `authHome` still runs on a bare inherited environment, and awewarm's own requests stay direct with proxy variables ignored. No `all_proxy` and no `NO_PROXY` are injected — CLI traffic speaks HTTP(S), and one switch means one route. Server-side delegation follows the same machine-local rule it already had: a serve/hub box's CLI-mode fires now consume that box's `config proxy`, like its subscription and native fires; failure hints name the shared egress (still never the URL itself — one can carry credentials).

## v0.6.6

**awewarm's own requests go direct and ignore proxy environment variables; a network that needs a proxy opts in explicitly.** Whether traffic needs a proxy is a property of the destination and the local network, which machine-wide `http_proxy`/`https_proxy`/`all_proxy` variables (typically set for git/npm or a desktop Clash) cannot express — and Python's urllib read them for everything, so a warm-up system's control channel inherited a proxy's failure domain: a Clash route gone bad made `awewarm status` report the hub unreachable (and could take warm-ups with it) while direct access worked. All of awewarm's own HTTPS — the hub control channel, subscription warm-ups, native account fires, update checks — now egresses direct through a new `net` module whose opener is built with no proxy logic at all, making the interactive CLI behave exactly like the background tick (which runs without a shell environment). A network that genuinely requires egress through a proxy opts in with `awewarm config proxy <url>` (stored as the top-level `proxyUrl`, validated at load and save, `none` clears; `awewarm config proxy` shows the current egress); CLI subprocesses (`codex`/`claude`) are outside the policy and keep the ambient environment. Network-level failures now append an egress hint naming the route actually used (never the proxy URL — one can carry credentials), so a broken route says which side to fix. The same rule binds a `serve`/hub box: its warm-up fires read the `proxyUrl` of the config on that machine (`awewarm config proxy` there — the fix its own failure hint names), and a config that is absent or unreadable means direct, never a dead fire. The scheme check accepts any case (`HTTP://host` is as valid as `http://host`, RFC 3986).

**Several logins of one provider can warm side by side — local or delegated.** Account connections gain an `authHome` field: the CLI config dir the connection logs in from. Official accounts managed by aweswitch (`~/.config/aweswitch/accounts/<provider>/<name>/`, each a private CLI config dir holding its own login file) are now discovered as their own findings — `Codex (cxo-heck)`, `Codex (cxo-peng)`, `Claude Code (work)` — in `awewarm discover`, `config add`, and `init`; adding one stores the dir as the connection's `authHome`. Locally the CLI subprocess is pointed at it (codex: `CODEX_HOME`; claude: `CLAUDE_CONFIG_DIR` plus `CLAUDE_CODE_DONT_USE_KEYCHAIN=1`, the same env aweswitch launches with, so the dir's file-based login wins over the machine-wide Keychain). Delegated, the push reads the login from that dir (authHome before `CODEX_HOME`/Keychain), and fingerprint-drift re-push stays per connection — accounts rotate independently. The server side is untouched: its per-connection sandbox and native fires already key off the pushed credential. An absent `authHome` is exactly the old behavior (`~/.codex`, the Keychain or `~/.claude`). The field is validated at load and save (a non-empty path, account connections only), and a claude account dir without `.credentials.json` (no file login yet) is not listed.

**macOS: the tick's self-heal could take the scheduler down with it.** The self-heal at the top of every tick reinstalls the launchd job when its calendar entries no longer match the config — exactly what happens the first time a connection moves back local or a local slot changes. But the tick process IS the launchd job: the reinstall's `bootout` terminated the caller mid-heal, so the `bootstrap` reload never ran and the scheduler died silently (local slots stopped firing; observed on a real install the moment two codex accounts went local). The heal's reinstall now goes through a detached shell that waits for the tick to exit before swapping (`bootout`, then `bootstrap` with legacy `load` as fallback) — a mid-pass swap can also no longer kill a due activation after its request went out. Interactive `scheduler install` keeps its synchronous, error-reporting behavior.

## v0.6.5

**The server protects itself from overload and accepts every connection it already had.** A `BoundedThreadingHTTPServer` caps concurrent request threads at 64 and returns a clean `503 Service Unavailable` the moment the ceiling is hit — a looping or storming client can no longer monopolise the process. Connection IDs are validated against path traversal and control characters, and `PUT`/`DELETE` handlers reject malformed ids before they reach the config layer. The request body parser now rejects non-object JSON payloads and negative `Content-Length` values, closing a class of input-forgery bugs.

Fixed-mode slots now wrap around midnight correctly. `grid_times` walks a full 24-hour cycle (not just the anchor's calendar day), and `_due_fixed` re-checks yesterday's slots while any are still inside the catch-up window — a late fire after midnight no longer leaves a cold gap until morning. `record_success` and `record_skip` record against the slot's own calendar day (`slotAt`) rather than `now`, so cross-midnight catch-up lands in the right row.

## v0.6.4

**Delegated accounts can warm natively over HTTPS when the CLI is not installed on the server.** A push of an account connection to a box without claude/codex no longer fails: the server records `transport.exec = "native"` and fires through `transport.send_native` — codex gets a ChatGPT backend SSE request built from its OAuth tokens, claude gets an HTTPS POST carrying its `accessToken`, both shaped in pure builders (`native_request_parts`) that never touch the wire until send time. A later repush flips the connection back to CLI mode when the binary has since appeared (`_resolve_cli` returns None instead of failing; the resolved mode rides the push response and shows in `status` as native vs CLI). The push validates the credential offline first (`transport.validate_native_credential`): a delegation whose first scheduled tick is guaranteed to fail is rejected at accept time with a 400 naming the fix (`run \`codex login\` / \`claude /login\` on the local machine, then: awewarm remote push`), and the same concrete login command replaced the old vague "log in again" wording in every credential-format error and in the 401 detail a native fire reports. The native fire treats a truncated or reset SSE stream after HTTP 200 as success (the activation was accepted) rather than an error — `readline` is bounded by `NATIVE_SSE_CAP_BYTES` so a misbehaving peer cannot balloon memory, and `IncompleteRead`/`HTTPException` join the tolerated exceptions. `_resolve_cli` also understands Windows spellings pushed to a POSIX server (`C:\...\codex.cmd` resolves via basename and stem for `.bat`/`.cmd`/`.exe`/`.ps1`). The account-delegation test suite passes on Windows CI again (a mock-server `shutil.which` stub leaked between tests).

## v0.6.3

**CLI login accounts can be delegated to a remote server** — parity with API-key subscriptions. `awewarm config set <id> --remote` now works on a Claude Code or Codex account: the local machine reads its own CLI login (`credstore`: the macOS Keychain entry or `~/.claude/.credentials.json` for claude, `~/.codex/auth.json` for codex — CODEX_HOME honored), pushes the credential exactly like an API key (server RAM only; `--persist-key` works too, with a harder-worded notice), and the server runs the CLI with the credential injected: claude gets `CLAUDE_CODE_OAUTH_TOKEN`, codex gets a per-connection `CODEX_HOME` sandbox whose `auth.json` is re-written from the push before every fire — any token refresh the server-side CLI performs is discarded, because the local login is the source of truth. Because a login credential is account-wide (every subscription under it), delegating an account adds an explicit confirmation naming the target server URL (`--yes` for non-interactive shells). `awewarm remote push` re-reads the login each time, and the background sync (at most twice an hour) re-pushes automatically when the credential's fingerprint no longer matches what the server holds — the fingerprint rides the push body (`credentialFingerprint`) and shows in `awewarm status <id>` (`credential: server RAM only, fingerprint ...`). Prerequisite: the server box needs the CLI installed — the push resolves the binary server-side (stored path, else PATH by basename) and fails actionably naming it otherwise. Wire: `PUT /v1/connections/<id>` accepts `kind: "account"` plus the fingerprint; `/v1/state` reports it; `/v1/keys` is unchanged (credential-agnostic). Takeback deletes the codex sandbox; `config.json` on the server records fingerprints under `credentialFingerprints`.

**`serve` no longer runs activations under its global lock.** The tick snapshots connections under the lock, then each connection's plan+fire runs under its own mutex on a bounded 4-worker pool, re-taking the lock once at the end to persist state — one slow CLI activation (delegated accounts run at the 120 s CLI cap; HTTP stays at 15 s) can no longer stall `/v1/state`, pushes, or other connections. `run_now` uses the same per-connection path, so a tick and a manual run of one connection can no longer overlap. `awewarm run <id>` on a delegated account waits out the CLI cap (client timeout 135 s) instead of reporting a phantom failure at 30 s.
...
**Opt-in server-side key storage (`--persist-key`), off by default and discouraged.** Delegated connections' API keys live in the server's RAM only, so a server restart while your machine is offline holds that connection's warm-ups until the machine returns. For machines that are rarely online, `awewarm config set <id> --persist-key on` opts a connection into server-side storage instead: the key lands in `keys.json` inside the server's data dir (plaintext, 0600 — the same trust level as a hub's tenants.json) and survives restarts with no re-push. It is deliberately friction-bound: every command that starts or stops persistence confirms first — turning it on defaults to No (and `--yes` is the only non-interactive escape), turning it off defaults to Yes but states both consequences (the server deletes the key at once; from then on a restart while this machine is offline holds warm-ups again). Delegating a flagged connection, `--duplicate --remote`, and restoring a backup that contains flagged connections all gate the same way; the background sync never prompts. On a hub the operator must also allow it first (`awewarm-hub config --persist-keys on`, default off — hubs 403 persisted pushes otherwise). `awewarm status` labels each side (`key: server RAM only` / `key: stored on the server`). Wire: `PUT /v1/connections/<id>` gains an optional `persistKey` (omitted when false, so old servers see the exact body they always did), `/v1/keys` is unchanged (the server writes through for its own persisted set), and the `/v1/state` view gains `keyPersisted`. Taking a connection back purges its disk copy.
